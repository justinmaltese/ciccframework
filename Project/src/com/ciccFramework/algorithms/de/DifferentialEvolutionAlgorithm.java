...
/* 
 * This class serves as a driver for the Differential Evolution Algorithm.
...
public class DifferentialEvolutionAlgorithm extends Algorithm {
	...
	public DifferentialEvolutionAlgorithm(CoveringProblem problem, ParameterSet params) {
	...
	private Agent createTrialPosition(int index, SolutionSet<Agent> population) {
		...
		//select 3 agents for crossover/mutation purposes
		...
		for (int i=0;i<problem.numDecisionVariables;i++) {
			double pos;
			//crossover + mutate if necessary
			if (random.nextDouble() < (Double)params.get("CROSSOVER_PROBABILITY") || i == randDecisionVariableIndex) {
				pos = agent1.getDecisionVariableAtIndex(i) + ((Double)params.get("DIFFERENTIAL_WEIGHT")*(agent2.getDecisionVariableAtIndex(i) - agent3.getDecisionVariableAtIndex(i)));
				pos = Math.round(pos);
