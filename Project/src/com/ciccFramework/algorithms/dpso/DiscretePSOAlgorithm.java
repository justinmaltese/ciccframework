...
/* 
 * This class serves as a driver for the Discrete Particle Swarm Optimization Algorithm.
...
public class DiscretePSOAlgorithm extends Algorithm {

	Particle globalBest;
	private Random random = new Random();

	
	public DiscretePSOAlgorithm(CoveringProblem problem, ParameterSet params) {
	...
	private void updatePositionAndVelocity(SolutionSet<Particle> swarm) {
		for (int i=0;i<(Integer)params.get("SWARM_SIZE");i++) {
			...
			for (int j=0;j<current.getNumDecisionVariables();j++) {
				...
				//perform mutation to bests if necessary
				if (this.random.nextDouble() < (Double)params.get("GBEST_MUTATION_PROBABILITY")) {
					current.setDecisionVariableAtIndexToValue(j, globalBest.getDecisionVariableAtIndex(j));
				} else if (this.random.nextDouble() < (Double)params.get("PBEST_MUTATION_PROBABILITY")) {
					current.setDecisionVariableAtIndexToValue(j, current.getPBestDecisionVariableAtIndex(j));
				}
			}
		}
	}
